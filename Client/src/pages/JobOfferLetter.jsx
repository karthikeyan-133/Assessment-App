...
const JobOfferLetter = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!name || !department || !companyName  || !photo) {
            setError('Please fill in all fields.');
            return;
        }
        setError('');
        console.log('Form submitted:', { name, department, companyName,  photo });
    };

    return (
        <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-800'} p-6`}>
            <h1 className="text-4xl font-bold text-center mb-6">Job Offer Letter</h1>
